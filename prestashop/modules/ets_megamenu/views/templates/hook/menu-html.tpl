...
{if isset($menus) && $menus}
  <ul class="mm_menus_ul">
    ...
    {foreach from=$menus item='menu'}
      <li  class="item_lv1 mm_menus_li {if $menu.custom_class}{$menu.custom_class|escape:'html':'UTF-8'}{/if} {if $menu.sub_menu_type}mm_sub_align_{strtolower($menu.sub_menu_type)|escape:'html':'UTF-8'}{/if} {if $menu.columns}mm_has_sub{/if}">
        <a href="{$menu.menu_link|escape:'html':'UTF-8'}">
          {if isset($menu.icon) && $menu.icon}<i class="fa {$menu.icon|escape:'html':'UTF-8'}" aria-hidden="true"></i>{/if}
          <span class="item_content">{$menu.title|escape:'html':'UTF-8'}</span>
